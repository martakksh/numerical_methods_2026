...
# генерую матрицю, щоб була з діагональним переважанням
...
# записую матрицю у файл, щоб потім звідти читати
def write_matrix(filename, A):
    with open(filename, "w") as f:
        for row in A:
            f.write(" ".join(map(str, row)) + "\n")

# зчитую матрицю з файлу
def read_matrix(filename):
    with open(filename, "r") as f:
        return [list(map(float, line.split())) for line in f]

# рахую вектор b, множачи матрицю на ідеальний розв'язок
...
# читаю вектор з файлу
def read_vector(filename):
    with open(filename, "r") as f:
        return [float(line.strip()) for line in f]

# шукаю максимальну різницю у векторі
def norm_vector(v):
    return max(abs(x) for x in v)

# шукаю норму матриці по рядках
def norm_matrix(A):
    return max(sum(abs(x) for x in row) for row in A)

# множу матрицю на вектор
def mat_vec(A, x):
    n = len(A)
    return [sum(A[i][j] * x[j] for j in range(n)) for i in range(n)]

# перевіряю, наскільки ми близькі до правди
def get_residual_norm(A, x, B):
    ax = mat_vec(A, x)
    res = [ax[i] - B[i] for i in range(len(B))]
    return norm_vector(res)

# метод простої ітерації (найпростіший підхід)
...
# метод якобі, де все оновлюється на наступному кроці
...
# метод зейделя, тут ми одразу використовуєм нові значення
...
def main():
    ...
    # зберігаю дані у файли, щоб бачити їх у проєкті
    ...
    # порівнюю, як працюють різні методи
    for name, func in methods:
        ...
        print(f"результат вже у {filename}")
